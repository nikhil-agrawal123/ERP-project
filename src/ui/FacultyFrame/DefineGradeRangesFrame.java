...
public class DefineGradeRangesFrame extends JFrame {
    ...
    private Color inputBgColor = new Color(40, 44, 55); // Darker for inputs
    ...
    public DefineGradeRangesFrame(String courseCode, String courseName) {
    ...
    private JScrollPane createMainContent() {
        ...
        gbc.insets = new Insets(8, 0, 8, 0); // Vertical spacing between rows
    ...
    private void addGradeRow(JPanel panel, GridBagConstraints gbc, GradeRange range) {
        ...
        // Special case for 'F' - usually fixed at 0
        if (range.getGradeLetter().equals("F")) {
            scoreField.setText("0");
            scoreField.setEnabled(false);
            scoreField.setBackground(cardColor); // Make it blend in
        ...
        scoreFields.add(scoreField); // Store for saving
    ...
    private void saveRanges() {
        ...
        // 3. Save to Backend (Call your service here)
        // gradingService.saveGradeRanges(courseCode, newRanges);

        JOptionPane.showMessageDialog(this, "Grade thresholds updated successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
        dispose();
    }
    ...
    private void styleTextField(JTextField field) {
        ...
        field.setPreferredSize(new Dimension(80, 40)); // Comfortable click area
        field.setBackground(inputBgColor);
        field.setForeground(textColor);
        field.setCaretColor(accentColor);
        field.setHorizontalAlignment(SwingConstants.CENTER);

        // Subtle border that glows on focus? simple line for now
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new DefineGradeRangesFrame("CS101", "Intro to Programming").setVisible(true));
    }
}